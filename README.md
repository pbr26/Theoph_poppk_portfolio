...
Theophylline is a narrow-therapeutic-index bronchodilator with well-characterised one-compartment oral PK. The built-in `Theoph` dataset has rich serial sampling from 12 subjects after a single oral dose, with body weight as a covariate of interest — a benchmark dataset for PopPK methodology.
...
- Snapshots are taken after each phase. The compiled stack (`rxode2`/`nlmixr2`) is excluded from the lockfile until a trivial model compiles successfully.
